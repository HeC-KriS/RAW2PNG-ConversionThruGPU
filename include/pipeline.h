...
struct PipelineConfig {
    ...
    // to 1024 when the modern GPU-deflate path will be used (the device-
    // side prefix-sum scan's hard ceiling, and empirically far faster than
    // small strips for that path -- 32 rows measured ~1076 ms vs 1024 rows
    // ~181 ms), or 256 otherwise (legacy CPU-deflate default, unchanged).
    // An explicit --strip-height always wins over auto-selection.
    int  strip_height     = 0;
